...
class ThrottleBase:
  def __init__(self):
    global minthrottle, ch_th_seq, emergency_stop, frame
  ...
  def resetThrottleInfo(self):
    ...
    global battery_count, ch_th_seq 

    # take about a second to find new minthrottle
    minthrottle = max(25, (minthrottle-1))
    maxthrottle = -1
  ...
  def adjustForBattery(self,pixPerFrame = -1):
    ...
    if pixPerFrame > 0 and pixPerFrame < DESIREDPPF and battery_adjustment < MAXBATADJ and maxthrottle > 0:
      battery_adjustment += .01
      print("BATTERY ADJUSTMENT %f" % battery_adjustment)
      battery_count = 0
    else:
      battery_count += 1
      if battery_count > MAXBATCNT:
        print("BATTERY ADJUSTMENT %f" % battery_adjustment)
        battery_count = 0
        battery_adjustment += .01
  ...
  def checkThrottle(self,expected_throttle):
      ...
      if ch_th_seq % CHECK_THROTTLE_THRESH == 0 and expected_throttle >= minthrottle / 100:
        check_throttle = True
...
class LaneLines:
  ...
  def get_map_data(self):
        global pixPerFrame, laneWidth, donkeyvpangle, dklinept, bestll, bestcl, bestrl, width
        return laneWidth, pixPerFrame, donkeyvpangle, bestll, bestcl, bestrl, width
