...
#    # print("lanewidths: %f, %f = %f + %f, %f angle %f" % (lanewidth1, lanewidth2 + lanewidth3, lanewidth2, lanewidth3, lanewidth1/2, donkeyvpangle))
  ...
  def setMinMaxThrottle(self,new_frame):
    global minthrottle, maxthrottle, frame, check_throttle
    if frame is None:
      frame = new_frame
      print("frame is None")
      return False
  ...
  def setSteerThrottle(self, pos, ll, cl, rl, conf):
    ...
    global minthrottle, maxthrottle 
    ...
    # print("angle %d" % angle)
    # print(cl)
    # steering = (cl[1][0] - midpix) / denom
      steering = (self.closestX(cl) - midpix) / denom
      if steering > 1:
        steering = 1
      elif steering < -1:
        steering = -1
      if angle < 60:
        ...
        steering = -1
    # print("cl x1 %d x2 %d lw %d" % (cl[0][0], cl[1][0], laneWidth))
      '''
      elif angle < 90:
        steering = min((90 - angle),denom) / denom
      elif angle >= 90:
        steering = max((90 - angle), -1*denom) / denom
      '''
      ...
      angle = self.orientation(ll)
    # print("ll angle %d" % angle)
      if angle < 60 or angle > 120:
        steering = 1
      # elif angle < 90:
        # steering = 1 * (90 - angle)/ 90
      else:
        if (ll[0][0] < ll[1][0]):
          steering = ( ((ll[1][0] + laneWidth/2) - midpix) / 80)
        else:
          steering = ( ((ll[0][0] + laneWidth/2) - midpix) / 80)
        # steering = ( ((self.closestX(ll) + laneWidth/2) - midpix) / 80)
    # print("ll x1 %d x2 %d lw %d" % (ll[0][0], ll[1][0], laneWidth))
    elif rl is not None :
      angle = self.orientation(rl)
    # print("rl angle %d" % angle)
      # if angle < 45 or angle > 135:
      if angle < 60 or angle > 120:
        steering = -1
      # elif angle > 90:
        # steering = -1 * (angle - 90)/ 90
      else:
        if (rl[0][0] > rl[1][0]):
          steering = ( ((rl[1][0] - laneWidth/2) - midpix) / 80)
        else:
          steering = ( ((rl[0][0] - laneWidth/2) - midpix) / 80)
        # steering = ( ((self.closestX(rl) - laneWidth/2) - midpix) / 80)
    # print("rl x1 %d x2 %d lw %d" % (rl[0][0], rl[1][0], laneWidth))
    '''
  ...
  def lrclines(self, lines, roi):
    ...
    # print("len ll %d cl %d rl %d"%(lllen, cllen, rllen))
      ...
      if done:
        ...
        x,y, dkangle, lanewidth = self.vanishing_point2(bestrl,bestll)
              ...
              if bestcl is None or 120 > cldeg > 60:
                ...
                # print("possibe vp2")
                  ...
                  # print("found possible best cl, ll")
                    lwmaxdif = min(abs(lanewidth - laneWidth), abs(lanewidth/2 - laneWidth))
                    if lwmaxdif == abs(lanewidth/2 - laneWidth):
                      bestcl = None
                      bestrl = cl
                    else:
                      bestcl = cl
          ...
          if (rllen >= 1):
            for rl in currline:
              ...
              if bestcl is None or 120 > cldeg > 60:
                ...
                if (VPy is None or besty > abs(VPy - y)) and y <= 0:
                # print("Possibe vp2")
                  if laneWidth < 0 or abs(lanewidth - laneWidth) < lwmaxdif or abs(lanewidth/2 - laneWidth) < lwmaxdif:
                    lwmaxdif = min(abs(lanewidth - laneWidth), abs(lanewidth/2 - laneWidth))
                  # print("Found possible best cl, rl")
                    if laneWidth > 0 and lwmaxdif == abs(lanewidth/2 - laneWidth):
                      bestcl = None
                      bestll = cl
                    else:
                      bestcl = cl
      ...
      self.conf, self.steering, self.throttle = self.setSteerThrottle(curpos, lline[2], cline[2], rline[2], conf)

    # print ("steer %f throt %f conf %d pos(%s)" % (self.steering, self.throttle, self.conf, self.strpos(pos[2])))
      #######################
      # print to screen
      #######################
    ...
    # print("wrote %s" % (out))
      ...
      CHECK_THROTTLE_THRESH = 10
      if seq % CHECK_THROTTLE_THRESH == 0 and self.throttle >= minthrottle:
  ...
  def test_tub(self, tub_path):
        ...
        for clip in clips:
          for imgseq in clip:
            ...
            lines, roi = self.process_img(img, mode)
            # roi = self.getROI(img)
            if lines is not None:
...
global bestrl, bestll, bestcl, bestvx, bestvy, curconf, curpos
global minthrottle, maxthrottle, frame
global check_throttle
check_throttle = True
bestrl = None 
bestll = None
bestcl = None
