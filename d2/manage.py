...
def drive(cfg, model_path=None, use_joystick=False):
    ...
    # ARD: See if we should run in ReinforcementLearning module.
    if model_path == 'rl':
      kl = RL()
      # model_path = cfg.RL_MODEL_PATH
      kl.load(model = 'control')
    # ARD: run in pilot model.
    else:
      kl = KerasCategorical()
      if model_path:
        kl.load(model_path)
    ...
    steering = PWMSteering(controller=steering_controller,
                                    ...
                                    straight_pulse=cfg.STEERING_STRAIGHT_PWM, 
    ...
    inputs=['cam/image_array', 'pilot/angle', 'pilot/throttle', 'user/mode']
...
def train(cfg, tub_names, model_name):
    ...
    y_keys = ['pilot/angle', 'pilot/throttle']

    def rt(record):
        # record['user/angle'] = dk.utils.linear_bin(record['user/angle'])
        record['pilot/angle'] = dk.utils.linear_bin(record['pilot/angle'])
        return record

    # kl = KerasRLCategorical()
    kl = KerasCategorical()
